...
//제어하는 역할, 만일 Service존재가 없다면, 컨트롤러가 모델 영역의 목잡한 처리를
...
public class JdbcEmpService {
	...
	public int regist(Emp emp) {
		...
		try {
			...
			jdbcDeptDAO.regist(emp.getDept());//부서 등록, 이 메서드는 SQLException을 발생시킬수 있다
			jdbcEmpDAO.regist(emp);//사원등록
			con.commit();//위 두개의 insert업무 수행중 에러가 없다면 트랜잭션 커밋
			result =1;
		...
		poolManager.freeConnection(con);//반납
		return result;
	}
